...
import styles from '../style/MarketSummary.module.scss';
        ...
        <div key={index} className={styles.marketItem}>
          ...
          <div className={`${styles.row} ${styles.row1}`}>
            ...
            <div
              ...
              style={{ color: index === 2 ? '#99A1B7' : '#17C653' }}
            >
              {item.value}
            </div>
          </div>
          ...
          <div className={`${styles.row} ${styles.row2}`}>
            <div className={styles.shares}>{item.shares}</div>
            <div className={styles.volume}>{item.volume}</div>
          </div>

          {/* Hàng 3: status */}
          <div className={`${styles.row} ${styles.row3}`}>
            <div className={styles.statsIcons}>
              <img src={item.up} alt="Up" className={styles.icon} />
              <span style={{ color: '#17C653' }}>{item.one}</span>
              <span style={{ color: '#9E63FF' }}>{item.two}</span>
              <img src={item.vuong} alt="Vuong" className={`${styles.icon} ${styles.vuongIcon}`} />
              <span style={{ color: '#F6B100' }}>{item.three}</span>

              <img src={item.down} alt="Down" className={styles.icon} />
